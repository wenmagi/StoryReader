...
import com.anonymouser.book.CustomFont;
...
public class UserInfo {
    ...
    @Convert(converter = CustomFontConverter.class, columnType = String.class)
    ...
    @Generated(hash = 997153542)
    public UserInfo(boolean isDay, float baseTextSize, float coefficient, float textSize,
            int moonBgColor, int moonTextColor, int dayBgColor, int dayBgImg, int dayTextColor,
            int light, CustomFont customFont, float lineHeight, boolean isSimple) {
        this.isDay = isDay;
    ...
    public static class CustomFontConverter implements PropertyConverter<CustomFont, String> {

        @Override
        public CustomFont convertToEntityProperty(String databaseValue) {
            for (CustomFont font :
                    CustomFont.values()) {
                if (font.getPath().equals(databaseValue))
                    return font;
            }

            return CustomFont.方正楷体;
        }

        @Override
        public String convertToDatabaseValue(CustomFont entityProperty) {
            return entityProperty == null ? null : entityProperty.getPath();
        }
    }
